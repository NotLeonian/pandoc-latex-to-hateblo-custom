...
class hatena_token():
    ...
    def _wsse(self, hatena_user=None, foto_api_key=None):
        ...
        b_digest = hashlib.sha1(
            b_nonce + created_at.encode() + foto_api_key.encode()).digest()
    ...
    def post_hatenaphoto(self, file_name, title='', to_folder=None, *args):
        ...
        r = requests.post(self.endpoint_photo, data=self._create_image_xml(
            file_name_path, title, to_folder), headers={'X-WSSE': self._wsse(*args)})
        try:
            r.raise_for_status()
        except:
...
def filter_hatena_space(elem, doc):
    ...
    if isinstance(elem, pf.Str):
        ...
        text = re.sub(
            r'(?:(?<=\s)|(?<=^)|(?<=[^\x00-\x7F]))([+-]?\d+(\.\d+)?)(?:(?=\s)|(?=$)|(?=[^\x00-\x7F]))', r'[tex: \1]', text)
        return pf.Str(text)
...
def filter_hatena_footnote(elem, doc):
    ...
    if isinstance(elem, pf.Note):
        content_without_code = [pf.Str(f'`{pf.stringify(x)}`') if isinstance(
            x, pf.Code) else x for x in elem.content[0].content]
    ...
    if isinstance(elem, pf.Para) and any(isinstance(content, pf.Math) and content.format == 'DisplayMath' for content in elem.content):
        ...
        for content in elem.content:
            if isinstance(content, pf.Math) and content.format == 'DisplayMath':
                result_elem.content.append(pf.RawInline(
                    '<div class="Math DisplayMath" style="text-align: center;"> ' + convert_math_symbols(content.text, True) + ' </div>'))
            else:
...
def filter_hatena_image(elem, doc):
    ...
    if isinstance(elem, pf.Image):
        if enable_upload:
            ...
            settings_root = Path(__file__).resolve(
            ...
            if path_settings is not None:
                ...
                params_hatenaapi = {k: os.environ.get(k) for k in (
                    'FOTO_API_KEY', 'HATENA_USER', 'HATENA_BLOG')}
            for k in ('FOTO_API_KEY', 'HATENA_USER', 'HATENA_BLOG'):
                if params_hatenaapi.get(k) is None:
                    raise KeyError(
                        f'API Parameter `{k}` not found in the settings.')
            ...
            image_file_id = ':'.join(res.find('hatena:syntax', {
                                     'hatena': 'http://www.hatena.ne.jp/info/xmlns#'}).text.split(':')[2:-1])
            image_file_id = f'[f:id:{image_file_id}:plain]'  # TODO: 画像サイズ調整
        else:
            image_file_id = 'INSERT_FILE_ID_HERE'
        ref_ids_img = [x.identifier for x in elem.content if hasattr(
            x, 'identifier') and x.identifier[:4] == 'fig:']
        if len(ref_ids_img) > 0:  # TODO: 複数ID存在するケースってあるの
            caption_inlines = [
                x for x in elem.content if isinstance(x, pf.Inline)]
            caption_inlines = [x for x in caption_inlines if not (
                hasattr(x, 'attributes') and x.attributes.get('label'))]
...
def filter_table_remove_tag(elem, doc):
    ...
    if isinstance(elem, pf.Table):
        ref_ids_tab = [x.identifier for x in elem.caption if hasattr(
            x, 'identifier') and x.identifier[:4] == 'tab:']
        if len(ref_ids_tab) > 0:
            tab_caption_inlines = [
                x for x in elem.caption if isinstance(x, pf.Inline)]
            tab_caption_inlines = [x for x in tab_caption_inlines if not (
                hasattr(x, 'attributes') and x.attributes.get('label'))]
