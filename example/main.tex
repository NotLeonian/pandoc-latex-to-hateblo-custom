...
最近はてなブログの更新がめんどくさくなってきたので効率化をはかる. 一部の要素をはてな記法に変換し, texファイルからはてな記法対応
...
は人によっては違う形式になるはず. \texttt{FOTO\_FOLDER} は画像のアップロード先.
...
で, 以下のようなコマンドで変換する. \texttt{{[}...{]}} はオプション.
...
\texttt{/} の \texttt{settings.json}, (3) 環境変数. で優先される.
...
\end{itemize}
\end{itemize}
なお, このエントリも LyX で書いて変換したものをそのまま投稿している. 原稿は\href{https://github.com/Gedevan-Aleksizde/pandoc-hateblo/doc}{ここ}

\section{やり方}

\subsection{インストール}

まずは pandoc を用意する. Ubuntu のレポジトリだとバージョンが少し古いので \href{https://github.com/jgm/pandoc/releases}{github}
で \texttt{.deb} を落としてきてインストールする. 関連パッケージとの対応を考慮して最新版より少し古い 2.9.2.1
を使用した. このへんバージョン制約が多いみたいなので注意. Pandoc 2.9.2.1 に対応する \texttt{\href{https://github.com/lierdakil/pandoc-crossref}{pandoc-crossref}}
の最新バージョンは 3.6.4 だが, これを少しだけいじったやつを使う (理由は後述). \texttt{stack} のほうは
ver. 2.3.3
...
はそのうち覚えたいが……}. その結果, オリジナルとは全く別物になった. なお pandoc を 2.9にしたのは \texttt{panflute} が対応していない\footnote{https://github.com/sergiocorreia/panflute/issues/142}ため.

\href{https://github.com/Gedevan-Aleksizde/pandoc-hateblo}{:embed:}

\subsection{pandoc の処理の概要}

pandoc の内部での処理フローは, (1) ソースを解析する (2) 中間ファイルとして抽象構文木に変換する (3) 指定したフォーマットに再変換する,
という処理をしており, フィルタは (2) の抽象構文木を修正する処理のことになる\footnote{https://pandoc.org/filters.html}.
...
("", [], [("<key>", "<value>"), (...)])
\end{lstlisting}
\texttt{panflute} では, \texttt{.format}, \texttt{\textless element\textgreater .attributes}
でアクセスできる. \texttt{collections.OrderedDict} 型. ただし平文 (\texttt{Str}
など) にはこの属性はない.\texttt{ \textless content\textgreater} には (1) ダブルクオーテーションで括った文字列,
(2) 構文木の入れ子, (3) それらを \texttt{{[}}, \texttt{{]}}で括ったリスト, のどれかを与えられる.
\texttt{ELEM.content} でアクセスできる. これは \texttt{list} 型.

構文木入れ子の最上層の型は \texttt{Pandoc} であり, \texttt{\textless attributes\textgreater}
にメタ情報が入っている. ただしこれは \texttt{doc.get\_metadata()} でアクセスする.
...
\begin{lstlisting}[language=sh]
pandoc --mathjax --wrap=auto -f latex -t html  -F pandoc-crossref -F hateblo-filter.py -MfigureTitle='図' -MfigPrefix='図' -MtableTitle='表' -MtblPrefix='表' --bibliography=CITATIONS.bib --csl=CSL.csl -o blog/OUTPUT.html INPUT.tex
\end{lstlisting}
...
を再現するのは難しい. おそらくは biblatex で書いたほうがよそにも流用できて良さそうだが (Word を使う予定がないので
CSL は全く出番がない) である. あるいはフィルタとして処理を書くという方針でもできるだろうが, どちらも大変なので一旦保留.
